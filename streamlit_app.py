import streamlit as st
from streamlit_drawable_canvas import st_canvas
from keras.models import load_model
import numpy as np
import cv2
import os

# Set up page layout and design
st.set_page_config(page_title="Digit Recognizer", layout="wide")
st.title("🖌️ Handwritten Digit Recognizer")
st.write("Draw **one digit (0–9)** in the box and the model will predict it. Click **Clear** before drawing the next digit.")

# Load Keras Model
@st.cache_resource
def load_my_model():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return load_model(os.path.join(base_dir, "bestmodel.h5"))

try:
    model = load_my_model()
except Exception as e:
    st.error(f"Error loading model: {e}. Please ensure 'bestmodel.h5' is in the same directory.")
    st.stop()

# Session state key counter — incrementing this resets the canvas
if "canvas_key" not in st.session_state:
    st.session_state.canvas_key = 0

# Layout: canvas on left, prediction on right
col_canvas, col_pred = st.columns([3, 1])

with col_canvas:
    # Clear Canvas button above the canvas
    if st.button("🗑️ Clear Canvas"):
        st.session_state.canvas_key += 1

    # Configure the canvas parameters
    canvas_result = st_canvas(
        fill_color="rgba(0, 0, 0, 1)",  # Match black background — prevents white fill inside closed strokes
        stroke_width=8,
        stroke_color="#FFFFFF",
        background_color="#000000",
        height=300,
        width=900,
        drawing_mode="freedraw",
        key=f"canvas_{st.session_state.canvas_key}",  # Changing key resets the canvas
    )

with col_pred:
    st.markdown("<br><br>", unsafe_allow_html=True)

    # If canvas has drawing, run prediction
    if canvas_result.image_data is not None:
        img = canvas_result.image_data

        # Check if anything has been drawn (non-black pixels)
        if np.any(img[:, :, :3] > 0):
            # Convert RGBA canvas image to grayscale
            gray = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_RGBA2GRAY)

            # Crop to bounding box of drawn pixels
            coords = np.argwhere(gray > 0)
            if len(coords) > 0:
                y_min, x_min = coords.min(axis=0)
                y_max, x_max = coords.max(axis=0)

                # Expand crop with padding
                padding = 15
                h, w = gray.shape
                x_min = max(x_min - padding, 0)
                x_max = min(x_max + padding, w)
                y_min = max(y_min - padding, 0)
                y_max = min(y_max + padding, h)

                cropped = gray[y_min:y_max, x_min:x_max]

                # Pad to square BEFORE resizing — preserves digit aspect ratio to match MNIST
                crop_h, crop_w = cropped.shape
                diff = abs(crop_h - crop_w)
                if crop_h > crop_w:
                    pad_left = diff // 2
                    pad_right = diff - pad_left
                    cropped = np.pad(cropped, ((0, 0), (pad_left, pad_right)), 'constant', constant_values=0)
                elif crop_w > crop_h:
                    pad_top = diff // 2
                    pad_bot = diff - pad_top
                    cropped = np.pad(cropped, ((pad_top, pad_bot), (0, 0)), 'constant', constant_values=0)

                # Add a small border margin, resize to 28x28, normalize
                cropped = np.pad(cropped, 8, 'constant', constant_values=0)
                final_image = cv2.resize(cropped, (28, 28)) / 255.0
                final_image = final_image.reshape(1, 28, 28, 1)

                # Predict
                predictions = model.predict(final_image)
                predicted_class = np.argmax(predictions)
                confidence = predictions[0][predicted_class]

                # Display Prediction in the right column
                st.markdown(
                    f"<h1 style='text-align: center; color: #FF4B4B; font-size: 72px;'>{predicted_class}</h1>",
                    unsafe_allow_html=True
                )
                st.markdown(
                    f"<h4 style='text-align: center;'>Confidence<br><b>{confidence:.2%}</b></h4>",
                    unsafe_allow_html=True
                )
        else:
            st.markdown("<p style='text-align:center; color:grey;'>Start drawing...</p>", unsafe_allow_html=True)
